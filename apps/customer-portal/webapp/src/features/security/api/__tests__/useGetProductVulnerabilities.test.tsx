...
import { useGetProductVulnerabilities } from "@features/security/api/useGetProductVulnerabilities";
...
describe("useGetProductVulnerabilities", () => {
  ...
  it("encodes vulnerability id in request URL", async () => {
    const payload = { id: "v-1", cveId: "CVE-1" };
    ...
    const { result } = renderHook(
      () => useGetProductVulnerabilities("XRAY-999 003"),
      { wrapper },
    );

    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(mockAuthFetch).toHaveBeenCalledWith(
      "https://api.test/products/vulnerabilities/XRAY-999%20003",
      { method: "GET" },
    );
  });
  ...
  it("throws when base URL is missing", async () => {
    ...
    const { result } = renderHook(() => useGetProductVulnerabilities("v-1"), { wrapper });
