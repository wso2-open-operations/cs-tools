...
describe("useGetProductUpdatesStats", () => {
  ...
  it("should return mock data when isMockEnabled is true", async () => {
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith(
      expect.stringContaining(
        "Fetching product updates stats for project ID: project-1, mock: true",
      ),
    );
  });

  it("should have correct query options", () => {
    renderHook(() => useGetProductUpdatesStats("project-1"), {
      wrapper,
    });
  ...
  it("should fetch from API when isMockEnabled is false", async () => {
    ...
    try {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(
          "Fetching product updates stats for project ID: project-1, mock: false",
        ),
      );
    } finally {
  ...
  it("should handle API error when isMockEnabled is false", async () => {
    ...
    try {
      ...
      expect(result.current.error?.message).toContain(
        "Error fetching product updates stats: Internal Server Error",
      );
  ...
  it("should not fetch if projectId is empty", () => {
    const { result } = renderHook(() => useGetProductUpdatesStats(""), {
      wrapper,
    });
