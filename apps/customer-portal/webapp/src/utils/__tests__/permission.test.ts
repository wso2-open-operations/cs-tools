...
describe("isProjectSuspended", () => {
  ...
  // A lapsed contract used to count as suspended, which locked customers out of
  // the project the moment it expired. Per customer request it no longer does.
  it("returns false for a project whose contract has ended", () => {
    expect(isProjectSuspended("Open")).toBe(false);
    expect(isProjectSuspended(null)).toBe(false);
  });

  it("returns false when closureState is Open", () => {
    expect(isProjectSuspended("Open")).toBe(false);
  });

  it("returns false when closureState is absent", () => {
    expect(isProjectSuspended(null)).toBe(false);
    expect(isProjectSuspended(undefined)).toBe(false);
  });
});
