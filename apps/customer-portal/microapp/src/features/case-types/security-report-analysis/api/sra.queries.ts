...
} from "@features/case-types/security-report-analysis/api/sra.api";

export const securityReportAnalysis = {
  get: (id: string) =>
    queryOptions({ queryKey: ["security-report-analysis", id], queryFn: () => getSecurityReportAnalysis(id) }),

  all: (id: string, body: GetCasesRequestDto = {}) =>
    queryOptions({
      queryKey: ["security-report-analysis", id, body],
      queryFn: () => getAllSecurityReportAnalysis(id, body),
    }),
  ...
  paginated: (id: string, body: GetCasesRequestDto = {}) =>
    infiniteQueryOptions({
      ...
      queryFn: ({ pageParam }) =>
        getAllSecurityReportAnalysis(id, { ...body, pagination: { ...body.pagination, offset: pageParam } }),
      initialPageParam: 0,
