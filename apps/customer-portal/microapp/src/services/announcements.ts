...
const getAllAnnouncements = async (id: string, body: GetCasesRequestDto = {}): Promise<PaginatedArray<CaseSummary>> => {
  const response = (
    await apiClient.post<CasesDto>(PROJECT_CASES_ENDPOINT(id), {
      ...body,
      filters: {
        ...(body?.filters ?? {}),
        caseTypes: ["announcement"],
      },
    })
  ).data;
...
const getAnnouncement = async (id: string): Promise<Case> => {
...
export const announcements = {
  get: (id: string) => queryOptions({ queryKey: ["announcements", id], queryFn: () => getAnnouncement(id) }),

  all: (id: string, body: GetCasesRequestDto = {}) =>
    queryOptions({
      queryKey: ["announcements", id, body],
      queryFn: () => getAllAnnouncements(id, body),
    }),

  paginated: (id: string, body: GetCasesRequestDto = {}) =>
    infiniteQueryOptions({
      queryKey: ["announcements", "paginated", id, body],
      queryFn: ({ pageParam }) =>
        getAllAnnouncements(id, { ...body, pagination: { ...body.pagination, offset: pageParam } }),
      initialPageParam: 0,
