...
import type { IdTokenClaims } from "@lib/userClaims";
...
// SCIM call this app deliberately doesn't request (see AppWithConfig.tsx).
// The ID token is already in storage; just decode it.
//
// `getDecodedIdToken` is captured via a ref instead of listed in the deps
// array because the SDK does not guarantee referential stability for it —
// putting it in the deps array would re-run the effect on every render and
// trigger repeated token decodes.
