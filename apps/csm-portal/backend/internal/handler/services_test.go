...
func TestSearchITServices(t *testing.T) {
	...
	t.Run("rejects body exceeding 1 MiB", func(t *testing.T) {
		h := NewITServiceHandler(&mockEntityITServiceClient{})
		r := withUser(httptest.NewRequest(http.MethodPost, "/services/search", strings.NewReader(strings.Repeat("x", maxRequestBodyBytes+1))))
		w := httptest.NewRecorder()
		h.SearchITServices(w, r)
	...
	t.Run("rejects invalid JSON body", func(t *testing.T) {
		h := NewITServiceHandler(&mockEntityITServiceClient{})
		r := withUser(httptest.NewRequest(http.MethodPost, "/services/search", strings.NewReader(`not-json`)))
		w := httptest.NewRecorder()
		h.SearchITServices(w, r)
	...
	t.Run("forwards body to upstream and returns 200 with response", func(t *testing.T) {
		...
		client := &mockEntityITServiceClient{
			searchITServicesFn: func(_ context.Context, body []byte) ([]byte, error) {
				capturedBody = body
				return []byte(`{"services":[{"id":"11111111-1111-1111-1111-111111111111","name":"Email"}],"total":1,"limit":10,"offset":0}`), nil
			},
		}
		h := NewITServiceHandler(client)
		r := withUser(httptest.NewRequest(http.MethodPost, "/services/search", strings.NewReader(reqPayload)))
		w := httptest.NewRecorder()
		h.SearchITServices(w, r)
	...
	t.Run("upstream errors are mapped correctly", func(t *testing.T) {
		for _, tc := range upstreamErrors("Failed to search IT services.") {
			t.Run(tc.name, func(t *testing.T) {
				...
				h := NewITServiceHandler(client)
				r := withUser(httptest.NewRequest(http.MethodPost, "/services/search", strings.NewReader(`{}`)))
				w := httptest.NewRecorder()
				h.SearchITServices(w, r)
