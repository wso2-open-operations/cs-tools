...
// Product is a supported platform.
type Product struct {
	ID           string `json:"id"`
	Code         string `json:"code"`
	Name         string `json:"name"`
...
//
// Product, stage and tier match an organisation when *any* of its pairings
...
//
// Product and stage together partition the queue — every pairing has exactly one
...
// SearchUsersResponse is the reply to POST /plg/users/search.
