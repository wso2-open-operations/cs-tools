...
import { Eye, Plus } from "@wso2/oxygen-ui-icons-react";
...
/**
 * "My" — the engineer's own KB articles across every lifecycle stage
 * (draft, pending review, published, retired). Every authenticated user
 * sees only their own articles here (authorId is fixed to the current
 * user); reviewing *other* people's submissions happens on the separate
 * "To Review" tab.
 */
export default function CsmKBArticlesListPage(): JSX.Element {
  ...
  }, [debouncedSearch, page, rowsPerPage, user?.id, stateFilter]);
  ...
  const kbOptions = useMemo(
    () =>
      Array.from(new Set(allArticles.map((a) => a.knowledgeBaseId))).map((id) => ({
        value: id,
        label: kbNameById.get(id) ?? id,
      })),
    [allArticles, kbNameById],
  );

  const articles = useMemo(
    () => allArticles.filter((a) => kbFilter.length === 0 || kbFilter.includes(a.knowledgeBaseId)),
    [allArticles, kbFilter],
  );
    ...
    <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 1 }}>
        <Typography variant="body2" color="text.secondary">
          Articles you've authored, across every lifecycle stage.
        </Typography>
        <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
          ...
          <Button
            ...
            onClick={() => navigate("/knowledge/my-articles/new")}
          >
            Create new
          </Button>
        </Box>
      ...
      <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
        ...
        <KBMultiSelectFilter label="Status" options={STATE_OPTIONS} values={stateFilter} onChange={setStateFilter} minWidth={180} />
      </Box>
              ...
              {isLoading || isFetching ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} align="center" sx={{ py: 4 }}>
                    <Typography variant="body2" color="text.secondary">
                      No articles yet. Click "Create new" to write your first one.
                    </Typography>
                  </TableCell>
                </TableRow>
              ) : (
                articles.map((a) => (
                  ...
                  >
                    ...
                    <TableCell>{formatDate(a.createdOn)}</TableCell>
                    <TableCell>{formatDate(a.updatedOn)}</TableCell>
                    <TableCell>{(a.updatedBy && nameById.get(a.updatedBy)) || "—"}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
