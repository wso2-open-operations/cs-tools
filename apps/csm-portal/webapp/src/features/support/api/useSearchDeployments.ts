...
export function useSearchDeployments(
  ...
  return useQuery<SearchDeploymentsResponse, Error>({
    queryKey: ["deployments-search", request],
    queryFn: async () => {
      const res = await authFetch(`${apiConfig.backendUrl}/deployments/search`, {
        method: "POST",
      ...
      return (await res.json()) as SearchDeploymentsResponse;
    },
