 ...
 */
// Token helpers stubbed for the IdP-neutral mock build. The chat WebSocket is
// not exercised in mock mode; real connections require BFF-issued tokens.
const getAccessToken = async (): Promise<string | undefined> => undefined;
...
export function useChatWebSocket(options: UseChatWebSocketOptions) {
  ...
  const wsBaseUrl = useMemo(() => {
    const raw = window.config?.CSM_PORTAL_CHATBOT_WEBSOCKET_URL?.trim();
    if (!raw) return "";
  ...
  const connect = useCallback(
    async (sessionId: string): Promise<void> => {
      if (!wsBaseUrl) {
        throw new Error(
          "CSM_PORTAL_CHATBOT_WEBSOCKET_URL is not configured",
        );
      }
