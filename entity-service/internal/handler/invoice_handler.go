...
// InvoiceHandler handles HTTP requests for the invoice resource, backed by the ServiceNow
// data source only.
type InvoiceHandler struct {
	svc service.InvoiceService
}

// NewInvoiceHandler constructs an InvoiceHandler with the given service.
...
func (h *InvoiceHandler) SearchInvoices(w http.ResponseWriter, r *http.Request) {
	var req domain.SearchInvoicesRequest
	if !decodeRequest(w, r, &req) {
		return
	}
	resp, err := h.svc.SearchInvoices(r.Context(), req)
	if err != nil {
...
func (h *InvoiceHandler) GetInvoice(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	invoice, err := h.svc.GetInvoiceByID(r.Context(), id)
	if err != nil {
		writeServiceError(w, r, err)
		return
	}
