...
// snSla mirrors a single SLA object in the Choreo GET /cases/{id}/slas response,
// which is returned as a bare JSON array.
type snSla struct {
	ID                     string  `json:"id"`
...
// normalizeSlaStage lowercases, trims, and replaces spaces with underscores to
...
func (s *snCaseService) GetCaseSlas(ctx context.Context, id string) (domain.CaseSlaListView, error) {
	token := middleware.UserIDTokenFromContext(ctx)
	if token == "" {
		return domain.CaseSlaListView{}, &apierror.UnauthorizedError{Msg: "x-user-id-token header is required"}
	}

	raw, err := s.client.Get(ctx, "/cases/"+uuidToSysid(id)+"/slas", token)
	if err != nil {
		return domain.CaseSlaListView{}, err
	}

	var snSlas []snSla
	if err := json.Unmarshal(raw, &snSlas); err != nil {
		return domain.CaseSlaListView{}, fmt.Errorf("sn get case slas: parse response: %w", err)
	}

	slas := make([]domain.CaseSla, 0, len(snSlas))
	for _, sla := range snSlas {
		slas = append(slas, domain.CaseSla{
			ID:                     sysidToUUID(sla.ID),
			Definition:             sla.Definition,
			Target:                 sla.Target,
			Stage:                  normalizeSlaStage(sla.Stage),
			StageLabel:             sla.StageLabel,
			HasBreached:            sla.HasBreached,
			BusinessTimeLeftLabel:  sla.BusinessTimeLeftLabel,
			BusinessElapsedLabel:   sla.BusinessElapsedLabel,
			BusinessElapsedPercent: sla.BusinessElapsedPercent,
			StartTime:              sla.StartTime,
			StopTime:               sla.StopTime,
		})
	}

	return domain.CaseSlaListView{
		CaseID: id,
		Count:  len(slas),
		Slas:   slas,
	}, nil
