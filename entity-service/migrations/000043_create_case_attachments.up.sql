...
-- case_attachment stores metadata only. File bytes for this data source live
...
CREATE INDEX IF NOT EXISTS idx_case_attachment_uploaded_by ON case_attachment(uploaded_by);

-- Composite index for the paginated per-case feed (most recent first).
CREATE INDEX IF NOT EXISTS idx_case_attachment_case_created ON case_attachment(case_id, created_on DESC);
