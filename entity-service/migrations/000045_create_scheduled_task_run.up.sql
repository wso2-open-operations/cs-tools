...
CREATE TABLE IF NOT EXISTS scheduled_task_run (
  ...
  first_attempted_on TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  last_attempted_on  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  succeeded_on       TIMESTAMPTZ,
  superseded_on      TIMESTAMPTZ,
...
-- (succeeded_on / superseded_on / next_retry_on) — same choice sla_clocks
...
CREATE INDEX IF NOT EXISTS idx_scheduled_task_run_open
  ...
  WHERE succeeded_on IS NULL AND superseded_on IS NULL;
