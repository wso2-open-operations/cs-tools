...
func TestGetAccount(t *testing.T) {
	...
	t.Run("rejects non-UUID account ID", func(t *testing.T) {
		h := NewAccountHandler(&mockEntityAccountClient{})
		r := httptest.NewRequest(http.MethodGet, "/accounts/acc-42", nil)
	...
	t.Run("passes ID to upstream and returns 200 with response", func(t *testing.T) {
		...
		r := httptest.NewRequest(http.MethodGet, "/accounts/"+accountID, nil)
...
func TestSearchAccounts(t *testing.T) {
	t.Run("rejects body exceeding 1 MiB", func(t *testing.T) {
		h := NewAccountHandler(&mockEntityAccountClient{})
		r := httptest.NewRequest(http.MethodPost, "/accounts/search", strings.NewReader(strings.Repeat("x", maxRequestBodyBytes+1)))
		w := httptest.NewRecorder()
	...
	t.Run("rejects invalid JSON body", func(t *testing.T) {
		h := NewAccountHandler(&mockEntityAccountClient{})
		r := httptest.NewRequest(http.MethodPost, "/accounts/search", strings.NewReader(`not-json`))
		w := httptest.NewRecorder()
		h.SearchAccounts(w, r)
		assertStatus(t, w, http.StatusBadRequest)
	...
	t.Run("forwards body to upstream and returns 200 with response", func(t *testing.T) {
		...
		h := NewAccountHandler(client)
		r := httptest.NewRequest(http.MethodPost, "/accounts/search", strings.NewReader(reqPayload))
		w := httptest.NewRecorder()
		h.SearchAccounts(w, r)
...
func TestSearchAccountContacts(t *testing.T) {
	...
	t.Run("rejects empty account ID", func(t *testing.T) {
		h := NewAccountHandler(&mockEntityAccountClient{})
		r := httptest.NewRequest(http.MethodPost, "/accounts//contacts/search", strings.NewReader(`{}`))
		w := httptest.NewRecorder()
	...
	t.Run("rejects non-UUID account ID", func(t *testing.T) {
		h := NewAccountHandler(&mockEntityAccountClient{})
		r := httptest.NewRequest(http.MethodPost, "/accounts/acc-42/contacts/search", strings.NewReader(`{}`))
		r.SetPathValue("id", "acc-42")
		w := httptest.NewRecorder()
		h.SearchAccountContacts(w, r)
		assertStatus(t, w, http.StatusBadRequest)
	...
	t.Run("rejects body exceeding 1 MiB", func(t *testing.T) {
		h := NewAccountHandler(&mockEntityAccountClient{})
		r := httptest.NewRequest(http.MethodPost, "/accounts/"+accountID+"/contacts/search", strings.NewReader(strings.Repeat("x", maxRequestBodyBytes+1)))
		r.SetPathValue("id", accountID)
		w := httptest.NewRecorder()
		h.SearchAccountContacts(w, r)
		assertStatus(t, w, http.StatusRequestEntityTooLarge)
	...
	t.Run("rejects invalid JSON body", func(t *testing.T) {
		h := NewAccountHandler(&mockEntityAccountClient{})
		r := httptest.NewRequest(http.MethodPost, "/accounts/"+accountID+"/contacts/search", strings.NewReader(`not-json`))
		r.SetPathValue("id", accountID)
		w := httptest.NewRecorder()
		h.SearchAccountContacts(w, r)
		assertStatus(t, w, http.StatusBadRequest)
	...
	t.Run("allows an empty body", func(t *testing.T) {
		client := &mockEntityAccountClient{
			searchAccountContactsFn: func(_ context.Context, _ string, body []byte) ([]byte, error) {
				if len(body) != 0 {
		...
		h := NewAccountHandler(client)
		r := httptest.NewRequest(http.MethodPost, "/accounts/"+accountID+"/contacts/search", nil)
		r.SetPathValue("id", accountID)
		w := httptest.NewRecorder()
		h.SearchAccountContacts(w, r)
		assertStatus(t, w, http.StatusOK)
	...
	t.Run("forwards body verbatim and returns upstream response", func(t *testing.T) {
		...
		h := NewAccountHandler(client)
		r := httptest.NewRequest(http.MethodPost, "/accounts/"+accountID+"/contacts/search", strings.NewReader(reqBody))
		r.SetPathValue("id", accountID)
		w := httptest.NewRecorder()
		h.SearchAccountContacts(w, r)
