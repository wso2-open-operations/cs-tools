...
// TestDoForwardsUserIDToken verifies an end-user identity token stored in the
// request context (via WithUserIDToken) is forwarded as x-user-id-token on the
// outgoing request — the optional pass-through used by callers that need
// entity-service's ServiceNow-backed operations.
func TestDoForwardsUserIDToken(t *testing.T) {
	...
	ctx := WithUserIDToken(context.Background(), wantToken)
	if _, err := client.SearchAccounts(ctx, []byte(`{}`)); err != nil {
		t.Fatalf("SearchAccounts() error = %v, want nil", err)
	}
	if gotToken != wantToken {
		t.Errorf("x-user-id-token = %q, want %q", gotToken, wantToken)
	}
}

// TestDoWithoutUserIDTokenOmitsHeader verifies no x-user-id-token header is sent
// when the context carries none — the common case, since this service is M2M-only
// and most callers never supply one.
func TestDoWithoutUserIDTokenOmitsHeader(t *testing.T) {
	var sawHeader bool
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if sawHeader {
		t.Error("x-user-id-token header sent with no token in context, want omitted")
	}
}
