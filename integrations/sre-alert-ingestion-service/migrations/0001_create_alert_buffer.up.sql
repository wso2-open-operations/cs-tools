-- Durable buffer for inbound alerts this service could not (yet) hand off
-- to csm-integration-service as a CSM incident. A row here must survive
-- this service's own restarts and a regional failover -- that durability
-- requirement is the entire reason this table exists in a dedicated
-- database, not an in-process queue.

-- Human-readable, ServiceNow-style sequential alert numbers (ALT0000001,
-- ALT0000002, ...), generated by this service's own dedicated Postgres --
-- never CSM's database. Becomes the externally-facing identifier for a
-- buffered alert: the dedup tag embedded in CreateIncidentRequest.Subject
-- (see internal/csmclient.DedupTag), the id returned from POST /alerts'
-- 202 response, and what gets logged as "the" alert reference.
CREATE SEQUENCE IF NOT EXISTS alert_number_seq START 1;

CREATE TABLE IF NOT EXISTS alert_buffer (
  -- The DEFAULT is a fallback only: internal/handler.CreateAlert now
  -- generates this id client-side (internal/idgen) and always supplies it
  -- explicitly on INSERT, because it must be embedded as the dedup tag in
  -- the row's own CreateIncidentRequest.Subject before the row is
  -- persisted (see internal/csmclient.DedupTag). Kept as a DEFAULT anyway
  -- so the column still self-populates for any future caller that inserts
  -- without supplying one. gen_random_uuid() is core Postgres since v13,
  -- not pgcrypto -- no CREATE EXTENSION needed (a prior version of this
  -- migration had one anyway; it was a no-op on PG13+ and fails outright
  -- on managed Postgres that doesn't allow-list pgcrypto, e.g. Azure).
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  -- The DB primary key stays `id` above -- every UPDATE/WHERE in
  -- internal/store/postgres.go keys off it. alert_number is purely an
  -- additional, externally-facing label; NOT NULL/UNIQUE from creation
  -- since this table has never shipped without it.
  alert_number    TEXT NOT NULL UNIQUE,
  received_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
  payload         JSONB NOT NULL,
  status          TEXT NOT NULL DEFAULT 'pending'
                  CHECK (status IN ('pending', 'delivered', 'escalated', 'failed')),
  retry_count     INT NOT NULL DEFAULT 0,
  last_attempt_at TIMESTAMPTZ,
  last_error      TEXT,
  incident_id     TEXT,
  escalated_at    TIMESTAMPTZ
);

-- Backs the worker's periodic scan for pending rows, oldest first.
CREATE INDEX IF NOT EXISTS idx_alert_buffer_status_received_at
  ON alert_buffer (status, received_at)
  WHERE status = 'pending';
